...
def calculate_activation_force(word_i, word_j, data_list, word_frequency_dict):
    '''
    计算 activation_force
    '''
    ij_frequency, ij_dist = get_ij_frequency_and_dist(word_i, word_j, data_list)
...
def get_ij_frequency_and_dist(word_i, word_j, data_list):
    '''
    计算两个单词同时出现的频率与平均距离
    '''
...
def get_ij_frequency_and_dist_from_word_list(word_i, word_j, word_list):
    '''
    根据一个句子的词列表, 计算单词i与单词j的共现词频、距离和、出现次数
    '''
